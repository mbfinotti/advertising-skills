...
5. **Walk the layers in fixed order** (below) using [references/layer-evidence.md](references/layer-evidence.md). For each layer record one of four states - `pass` (evidence rules it out), `FAIL` (evidence confirms it), `unknown` (evidence missing), `not applicable` - with its own severity and confidence. Never collapse to a binary: `unknown` is not `pass`, and an unchecked layer silently marked clean is the most common way audits go wrong.
