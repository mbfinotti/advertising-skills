...
Both failures point at the same root cause: negatives were added quickly from the report UI without checking match type or variant coverage, so the list _looks_ maintained (45 entries!) while the actual query patterns keep leaking through gaps the entries were never built to close. To stop the monthly Groundhog Day:
