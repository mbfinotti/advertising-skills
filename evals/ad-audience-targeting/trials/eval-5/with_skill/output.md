...
- **Meta policy.** Financial hardship is named explicitly as a sensitive trait: it cannot be targeted on *or inferred*, even from a public signal. An exclusion list built from inferred hardship signals is the same mechanism as an inclusion list built from them, just run in reverse. Same rule, same violation.
