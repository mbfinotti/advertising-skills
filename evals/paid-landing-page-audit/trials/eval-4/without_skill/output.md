...
- Don't delete copy — **compress it into scannable fragments**: short benefit bullets, icon rows, before/after pairs. That serves attention constraints *and* keeps the trust signals.
...
> *"We're running these changes as an A/B test. At current traffic (22K sessions/mo), we can reliably detect a 25%+ relative CVR lift within ~5 weeks. Reporting the measured result in [date], not a pre-launch estimate."*
