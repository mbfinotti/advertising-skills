...
## 2. Route
This session's task *is* the check-in — handled by this re-invocation of `mbfinotti/advertising-skills@advertising-kickoff`. Nothing hands off for "the check-in" itself; what follows is what to run next.
...
- **`mbfinotti/advertising-skills@paid-media-scaling`** — after `mbfinotti/advertising-skills@cac-roas-benchmark` (short-list #2) returns a healthy verdict *and* the flight ends.
