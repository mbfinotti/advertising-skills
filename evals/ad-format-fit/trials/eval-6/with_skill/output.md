...
**Fix:** don't launch the automated type yet. Run a standard, manually-structured Shopping/Search campaign first on this budget. Graduate to the automated multi-surface campaign once: the pixel has fired verified test conversions (browser-side _and_ server-side) for at least 1–2 weeks, budget can move above ~$50/day, and the catalog/creative set has grown closer to 30+/15+.
