# Hook Scoring — Brimstone Cold Brew Meta Feed Ads (Cold Traffic)

**Hook rate = 3-sec plays / impressions.** The only thing that matters here is: does the first frame + first beat of motion stop a thumb before any product pitch registers. Everything after second 3 (brewing demo, guarantee) is irrelevant to this metric.
...
**D (8.5) — winner.** Same footage as C (pale, watery coffee = instant self-recognition for anyone who's had a diluted cold brew — near-zero parse time, works muted). Copy is a single second-person imperative clause: lower reading load than C's two sentences, and "Stop doing X" is one of the most consistently high-hook-rate formats in DTC video (proven pattern: name the reader's bad behavior, imply a fix exists). Shortest path from frame-1 to thumb-pause.
...
C and D share identical footage — the only variable is copy, which is cheap to test rather than predict. My read favors D, but the margin (8.5 vs 8.0) is inside normal creative-testing noise. Also flag: hook rate is a vanity metric on its own — A could win on raw hook rate precisely because it attracts spectacle-seekers unrelated to coffee, which wouldn't show up until you look at CTR/CVR downstream.

## Recommended test plan

1. Launch **C and D as a head-to-head pair** (same downstream asset, isolates the copy variable) — this is your cheapest, highest-signal test.
2. Launch **A as a wildcard** in the same test — if it wins hook rate but underperforms on hook-rate-to-purchase, you've confirmed the "high hook rate, low relevance" risk and can retire it.
3. **Hold B out of cold-traffic testing.** Repurpose it for retargeting/warm audiences where "$34 kit, 10 seconds" is a closing argument, not an opener.
