...
Per Meta's own metric definitions, ThruPlays ÷ impressions is not hook rate — it's one of the two **hold-rate** constructions in circulation. ThruPlay counts completion _or_ 15+ seconds watched. That's a fundamentally later, different window than the ≈3-second opening the pre-ranking scores.
...
Pooling all five naively gives 3/5 top-half hits, which — for what it's worth — would already _clear_ the skill's stated pass bar (≥3 of 5). But that pooled number is built on contaminated data and shouldn't be trusted either way.
...
1. Hook-style attention metrics correlate **-0.19 with ROAS** in the sourced dataset (slightly the wrong direction). A tool built on these dimensions was never going to reliably call the outright winner — that's evidence about the ceiling of what _any_ pre-launch attention score can do, not a flaw specific to this account's execution.
