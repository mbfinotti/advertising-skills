# Ad Spend Allocation Review — Peltova Q4 Media Plan, November Sale Event

**Verdict: Do not sign off as written.** Two of the three mechanics are named failure modes in the allocation playbook, and the evidence offered for them ("blended ROAS looked great last year") is exactly the evidence type this process is built to reject. The fixed-budget discipline (staying inside $450K) is correct and stays. Below is why, and the corrected mechanics I'd sign off on instead.
...
## Why the three mechanics don't clear review

**1. "Raise every campaign 40% for the event window" — flat, not marginal**
...
## Corrected mechanics — what I'd sign off on

1. **Keep the $450K total and the two-week event window.** No objection to timing a push around the sale.
2. **Replace the flat 40% raise with a marginal-ranked bump.** Rank current campaigns by headroom (penetration bands for social/audiences, impression-share loss for search) and weight the event increase toward the lines with actual room, holding or trimming anything already near saturation. Cap any single line's step at the 15–20% default; anything larger needs its own hypothesis, decision date, and rollback threshold — the event itself isn't a blanket exemption from step-size discipline.
3. **Don't reverse the prospecting/retargeting ratio.** Hold prospecting at least at its current share through the event — the elevated-intent argument cuts toward funding new-visitor acquisition, not away from it. Fund any retargeting increase from the incremental pool, not by cannibalizing prospecting, and cap retargeting's share (the field consensus flags >40% of platform spend on retargeting as the signal to expand prospecting, not the reverse).
4. **Size the December pull explicitly, per line.** "Most from December lines" isn't a change packet. Name which December campaigns lose the $60K, confirm they clear payback and floor checks after the cut, and set a rollback trigger if December underperforms because of it.
5. **Get one incremental read this cycle.** This move is large enough (~13% of the Q4 budget moving in one week, plus a structural prospecting/retargeting shift) that a misread costs real money — that's exactly the trigger for promoting incrementality above its default rank. A simple geo or audience holdout during the two-week window (even 10–15% of spend held out) would tell you, for the first time, whether last year's "great blended ROAS" was incremental or pulled-forward demand. Cheap relative to the $60K+ already in motion, and it's a reusable asset for next year's event plan instead of another guess.

---

## Governance

$60K on a $450K quarterly budget is ~13% of the total moving in one step — above the ~10% threshold this process flags for a documented approval workflow, and the retargeting/prospecting reversal is a structural change, not a routine reweight, so I'd route the full packet (not just the dollar amount) through marketing+finance sign-off before it goes live, with the rollback thresholds pre-agreed rather than decided mid-event.

## Open questions — needed before I can give you real numbers instead of a mechanics review
