# Halyard Media — Paid Search Manager Hiring Loop (Redesign)

## What's broken today

- **5 rounds, no compression** → candidates drop out mid-process. Every extra week loses your best people to faster-moving competitors.
- **Panel 1 and Panel 2 test the same competencies** → wasted candidate time, no new signal, just more chances for the group's first impression to calcify.
- **Consensus score before anyone writes anything down** → classic groupthink. Whoever speaks first (usually the most senior or most confident voice) anchors everyone else. You're not aggregating independent judgment, you're aggregating one person's opinion with extra steps.
- **Converges on the most polished talker** → a 1–10 "gut score" with no rubric measures charisma, not the job. Paid search is a numbers-and-execution role; talking a good game in a room is a weak proxy for whether someone can actually run a $40K/mo account well.

These four problems are one problem: **you're measuring impression, not evidence, and you're measuring it too slowly.**
...
This replaces "does this person sound sharp" with "can this person actually do the job." It's also the fastest way to kill polished-talker bias: someone can charm a room and still turn in a mediocre account audit, and vice versa.

Score independently (see rubric below) by one PPC lead **before** that person ever talks to the candidate live — removes halo effect from a friendly screen call contaminating the technical read.
...
- **Wide disagreement is a signal to dig in, not average away.** If the technical interviewer says 4 and the client-interview owner says 2, that's real information (e.g., strong operator, weak communicator) — don't smooth it into a 3 and lose it.
- **Ban impression-only questions** in the debrief ("would you grab a beer with them," "did they seem sharp"). Every claim in calibration must cite a rubric line and evidence sentence.
- **Hiring manager owns the final call**, informed by the score sheet and debrief — not a forced-consensus number the panel votes on.

---

## Why this fixes your four problems

1. **Drop-off** → loop cut from 5 rounds to 2 interviews + 1 paid async task, 8-day target, timeline disclosed upfront.
2. **Redundant panels** → competencies split across stages, nothing scored twice.
3. **Groupthink** → independent written scores before any group discussion exists.
4. **Polished-talker bias** → work sample tests real output before anyone talks to the candidate; rubric scores specific behaviors with required evidence instead of an unstructured 1–10 vibe check; dinner (pure charisma theater) removed from the decision entirely.
