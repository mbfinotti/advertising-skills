...
🟡 **Keep separate: Premium tier.** Different product economics mean a different target CPA (~$120, not $40, given 3x margin). Merging it with standard-margin campaigns would mix conversion events with different values into one optimization signal, confusing Meta's bidding. At $190.91 it's still ~1.6x over its *own* correct target, so it needs work — but structurally it stays its own campaign (or moves to value-based/ROAS bidding later, which is a separate, bigger decision).
