...
Standard commercial ad libraries mostly show what's _currently_ live, with weak-to-no historical archive. Set a **weekly** re-check (same sheet, append new rows, flag `date first seen` = today for anything new) — anything less frequent and you'll miss short-lived creative tests entirely, which is often where the most interesting hooks show up before they get scaled into evergreen ads.
