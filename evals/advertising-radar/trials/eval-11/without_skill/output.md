I couldn't get web-fetch/search permission in this session, so the source checks below are based on the facts in your note, not a live re-crawl. Here's the review.

## 🔴 Do first (2 minutes)

**Delete Fenwick on Creative / drewfenwick.com now.** A domain that used to host an ad-creative blog and now shows a car-detailing shop isn't a redesign — that's an expired domain picked up by someone else. Nothing published there today has any connection to the person you originally subscribed to. Remove the entry; don't leave it "to check later."

## ⚠️ The real finding: your "probably fine" label is unverified, and it just failed once already

Studio Pellworth and Cormorant Weekly carry the same note — "couldn't reach site" — that Fenwick on Creative would have carried right before it turned into a car-detailing shop. "Couldn't reach" and "probably fine" are contradictory: an unreachable site is evidence of nothing, and you just found the counter-example. Treat both as **unverified, not fine**, until someone actually loads them this week. That's a 2-minute check each — do it before spending any more weekly time budget on either.

## The list has zero labels — which is why it drifted to 110 min/week
...
1. Cut Fenwick on Creative. (0 min)
2. Load Studio Pellworth and Cormorant Weekly once each; relabel ✅/❌ based on what you actually see. (4 min)
3. Read Loom & Ledger, The Ad Joinery, Marta Villanueva at normal depth. (~35 min, adjust once you have real per-source times)
4. Add the two changelog RSS feeds; don't read them, just have them ready for when something breaks. (5 min setup, once)
5. Leave a one-line note on the Google Shopping gap for whoever reviews this list next.