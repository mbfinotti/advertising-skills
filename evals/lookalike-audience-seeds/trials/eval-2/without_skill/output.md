...
6. _Then_ take your top 15% (or just upload the full qualifying customer file with value — Meta's value-based LAL doesn't require pre-filtering to a top percentile; you can let the algorithm weight the full base by value instead of truncating it, which often performs better than an arbitrary top-15% cutoff).
