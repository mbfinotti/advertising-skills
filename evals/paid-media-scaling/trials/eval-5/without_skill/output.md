...
- **Platform-reported ROAS and GA4 are both biased in ways that get worse as spend scales.** Meta and Google each attribute conversions using their own models, and both platforms tend to claim overlapping credit for the same conversion (double-counted in a naive "blended ROAS" sum). GA4 undercounts on top of that (consent mode, ITP/cookie loss, cross-device gaps). Neither source, alone or added together, tells you the *incremental* revenue a new dollar of spend produces — which is the only number that matters when deciding whether to keep scaling.
