...
- **Blended paid channels (Meta + Google + TikTok): WATCH, leaning UNHEALTHY** — rests on rung 1 (break-even) and rung 2 (own history). The MER you were handed ($0 TikTok) reads _healthy_; the moment TikTok's real spend is put back in, it reads _below break-even and accelerating downward_. The gap between those two readings is the actual finding.
...
2. **Own history — Apr–Sep.** Already deteriorating for 3 straight months before October: 2.30 → 2.28 → 2.26 (Jul–Sep). Mix-shift check: nothing in the Apr–Sep series suggests a channel-mix explanation for that decline — it reads as a genuine efficiency drift, not reallocation. The zero-filled October figure (2.31) would _break_ that trend upward, which is itself a tell, since nothing changed operationally to explain a reversal — the only thing that changed is that a real cost got dropped from the math. The renormalized figure (~~2.13) _extends and steepens_ the decline: it's a larger single-month drop (~~-0.13 to -0.18) than any month in the trailing six (largest prior move was -0.04).
